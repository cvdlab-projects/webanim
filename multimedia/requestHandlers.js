...
function encodeVideo(response, request) {
    ...
    form.parse(request, function(error, fields, files) {
        ...
        if (fields.videoName !== "") {
            ...
            console.log("Recreating image structure of " + frameNumber + " frames...");
            for (frameId = 0; frameId < frameNumber; frameId++) {
                console.log("Recreating frame number " + frameId);
                imageMap = [];
                for (pixelNumber = 0; pixelNumber < (frameWidth * frameHeight * 4); pixelNumber++) {
                    if (pixelNumber % 4 !== 3) {
                        imageMap.push(fields['capturedFrames[' + frameId + '][data][' + pixelNumber + ']']);
                    }
                }
